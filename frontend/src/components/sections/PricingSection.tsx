...
export const PricingSection: React.FC = () => {
  ...
  return (
    <section id="pricing" className="py-20 lg:py-32 px-4 sm:px-6 lg:px-8 bg-bg-gradient-to/50 dark:bg-slate-900/20 border-y border-border/50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          ...
          >
            All plans include unlimited users and branches. Choose the duration that fits your firm's growth strategy.
          </motion.p>
        </div>
                ...
                {plan.features.map((feature, idx) => (
                  <div key={idx} className="flex items-start gap-3">
                    <div className={`mt-0.5 rounded-full p-0.5 flex-shrink-0 ${
                      feature.included ? 'bg-success/10 text-success dark:bg-emerald-950/40 dark:text-emerald-400' : 'bg-muted text-text-sub/50'
                    }`}>
                      <Check className="w-3.5 h-3.5" />
                    </div>
                    <span className={`text-xs font-bold ${
                      feature.included ? 'text-foreground/80 dark:text-slate-300' : 'text-text-sub/40 line-through'
                    }`}>
                      {feature.text}
                    </span>
                  </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-sm text-text-sub font-medium">
                Flat pricing regardless of staff count or client volume. Scaling has never been this affordable.
              </p>
            </div>
          </div>
          <Link
