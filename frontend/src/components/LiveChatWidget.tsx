...
export const LiveChatWidget: React.FC = () => {
  ...
  const { isOpen, isOnline, isTyping, unreadCount, messages, toggleOpen, setOnline, setTyping, addMessage } =
    useChatStore();
        ...
        {isOpen && (
          ...
          >
            <header className="flex items-center justify-between border-b border-slate-100 px-4 py-3 dark:border-slate-800">
              <div>
                <p className="font-heading text-sm font-bold text-foreground">OOMS Advisory Desk</p>
            ...
            <div ref={panelRef} className="max-h-80 space-y-3 overflow-y-auto px-4 py-4">
              ...
              {isTyping && <p className="text-xs font-medium text-muted-foreground">Advisor is typing...</p>}
            </div>

            <form onSubmit={handleSubmit} className="flex gap-2 border-t border-slate-100 p-3 dark:border-slate-800">
              <input
                value={draft}
