...
import {
  ShieldCheck, RefreshCcw, Lock, Globe, Mail, Phone, MapPin,
  ChevronRight, BookOpen, Scale, ArrowRight, ExternalLink
} from 'lucide-react';
import { SEO } from '../components/SEO';

const BusinessPolicy: React.FC = () => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            ...
            <p className="text-text-sub text-lg sm:text-xl max-w-2xl leading-relaxed font-medium">
              Updated July 1, 2025. This document outlines our commitment to transparency, security, and professional integrity in every interaction.
            </p>
          </motion.div>
        </div>
      </section>
          ...
          <aside className="lg:col-span-3 hidden lg:block">
            <div className="sticky top-28 space-y-10">
                  ...
                  {sections.map((s) => (
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        <div className={`p-1.5 rounded-lg transition-colors ${activeSection === s.id ? 'bg-white/20' : 'bg-muted dark:bg-slate-800 group-hover:bg-border'
                          }`}>
                          {React.cloneElement(s.icon as React.ReactElement<any>, { className: 'w-4 h-4' })}
                        </div>
                        <span>{s.title}</span>
                      </div>
                      <ChevronRight className={`w-4 h-4 transition-all duration-500 ${activeSection === s.id ? 'translate-x-0 opacity-100' : '-translate-x-2 opacity-0'}`} />
                    </button>
                  ))}
                </nav>
              </div>
              ...
              <div className="bg-muted rounded-[40px] p-10 text-white relative overflow-hidden shadow-2xl shadow-primary-600/30">
                <h4 className="font-heading font-black text-lg leading-tight tracking-tight">Need Legal Clarification?</h4>
                <p className="text-primary-50 leading-relaxed text-base font-medium opacity-90">
                  Our compliance team is available to discuss enterprise licensing and data agreements.
                </p>
                <a
            ...
            {sections.map((section, idx) => (
              ...
              >
                <div className="flex items-center space-x-5 mb-8 ml-2">
                  <div className={`p-4 rounded-2xl ${section.color} border border-current/10 shadow-lg`}>
                    {React.cloneElement(section.icon as React.ReactElement<any>, { className: 'w-6 h-6' })}
                  </div>
                ...
                <div className="bg-surface border border-border rounded-[48px] p-8 sm:p-12 shadow-2xl shadow-foreground/5 hover:border-primary-500/30 transition-all duration-500 relative overflow-hidden group">
                  <div className={`absolute top-0 right-0 w-48 h-48 opacity-[0.04] transition-transform duration-1000 group-hover:scale-125 group-hover:rotate-12 ${section.color.split(' ')[0]}`}>
                    {React.cloneElement(section.icon as React.ReactElement<any>, { className: 'w-full h-full' })}
                  </div>
                  ...
                  {typeof section.content === 'string' ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                      {section.content.map((item, i) => (
                        <div key={i} className="flex items-start space-x-5 p-6 rounded-[24px] bg-muted/30 border border-border/50 group/item transition-all duration-500 hover:bg-background hover:shadow-xl hover:border-primary-500/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-10">
              ...
              >
                ...
                <div className="relative z-10 space-y-8">
                  <h3 className="text-3xl font-heading font-black tracking-tighter leading-tight">Compliance Status</h3>
                  <p className="text-primary-50 leading-relaxed text-base font-medium opacity-90">
                    Our software and services comply with applicable Indian IT laws, tax laws, and data privacy norms. We undergo regular audits to maintain ISO 27001 standards.
                  </p>
                  <div className="pt-6 border-t border-white/20 flex flex-col space-y-3">
                    <span className="text-[10px] uppercase font-black tracking-widest text-primary-200">Registration Details</span>
                    <span className="text-xs font-mono font-black tracking-wider bg-white/10 px-3 py-1.5 rounded-lg inline-block w-fit">CIN: U46512AS2024PTC026214</span>
                  </div>
                </div>
              </motion.div>
              ...
              >
                <h3 className="text-3xl font-heading font-black text-foreground tracking-tighter mb-8 leading-tight">Official Contact</h3>
                <div className="space-y-8">
                  <div className="flex items-start space-x-5">
                    ...
                    <div>
                      <h4 className="text-[10px] font-black text-text-sub uppercase tracking-widest mb-1.5">Registered Office</h4>
                      <p className="text-base text-foreground font-black leading-relaxed">H. No. 356, Vill. Nagajan Niz, Kharupetiaghat, Darrang, Assam 784115</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <div className="flex items-center space-x-5 p-4 rounded-2xl bg-muted border border-border hover:border-primary-500/20 transition-all duration-500 shadow-sm">
                      ...
                      <span className="text-sm font-black text-foreground truncate">contact@onesaas.in</span>
                    </div>
                    <div className="flex items-center space-x-5 p-4 rounded-2xl bg-muted border border-border hover:border-primary-500/20 transition-all duration-500 shadow-sm">
                      <Phone className="w-5 h-5 text-primary-600" />
                      <span className="text-sm font-black text-foreground">+91-7002695990</span>
                    </div>
                  </div>
                </div>
              </motion.div>
