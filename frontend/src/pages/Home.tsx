            ...
            <div className="relative rounded-[32px] overflow-hidden border border-border shadow-2xl dark:shadow-slate-950/60 bg-white/80 dark:bg-dark-card/80 backdrop-blur-2xl p-6 sm:p-8 select-none animate-float">
              ...
              <div className="grid grid-cols-2 gap-5 py-6">
                <div className="p-5 rounded-3xl bg-primary-50 dark:bg-primary-950/20 border border-primary-500/10">
                  <span className="text-[9px] uppercase tracking-[0.15em] text-primary-700/70 font-black block mb-1.5">Pending Returns</span>
                  <span className="text-3xl font-black text-primary-700 dark:text-primary-400 font-heading tracking-tight">142</span>
                </div>
                <div className="p-5 rounded-3xl bg-success/5 dark:bg-emerald-950/20 border border-success/10">
                  <span className="text-[9px] uppercase tracking-[0.15em] text-success/70 font-black block mb-1.5">Completion Rate</span>
                  <span className="text-3xl font-black text-success dark:text-emerald-400 font-heading tracking-tight">98.4%</span>
                </div>
              </div>

              {/* Mock Tasks lists */}
              <div className="space-y-4">
                <span className="text-[9px] font-black text-text-sub uppercase tracking-[0.2em] block mb-1">Live Filing Pipelines</span>
                {mockTasks.map((task, idx) => (
                  <div key={idx} className="flex items-center justify-between p-4 rounded-2xl bg-surface dark:bg-slate-950/30 border border-border hover:border-primary-500 hover:shadow-lg hover:shadow-primary-500/5 transition-all duration-300">
                    <div className="flex items-center space-x-4">
                      <div className={`w-2.5 h-2.5 rounded-full shadow-sm ${task.color.split(' ')[0]}`} />
                      <div>
                        <h4 className="text-xs font-black text-foreground leading-none mb-1.5">{task.title}</h4>
                        <span className="text-[10px] text-text-sub font-bold">{task.client}</span>
                      </div>
                    </div>
                    <span className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${task.color.split(' ')[1]} bg-muted dark:bg-slate-800/50 border border-border dark:border-slate-700`}>
                      {task.status}
                    </span>
                  </div>
                ))}
              ...
              <div className="absolute bottom-6 -right-2 sm:-right-4 py-4 px-5 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 shadow-2xl shadow-emerald-600/30 text-white flex items-center space-x-4 border border-white/10 max-w-[260px]">
                ...
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider">WhatsApp Sent</h4>
                  <p className="text-[10px] text-emerald-50 font-medium">Auto alerts sent to 45 clients</p>
                </div>
              </div>
                  ...
                  >
                    ...
                    <div className="space-y-4 bg-muted/50 dark:bg-slate-950/50 rounded-3xl p-8 border border-border dark:border-slate-900">
                      ...
                      {ind.features.map((feature, i) => (
                        <div key={i} className="flex items-center space-x-4 text-foreground font-black text-sm">
        ...
        <div className="max-w-7xl mx-auto space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
            { [
              ...
              <div key={idx} className="group relative space-y-5 bg-surface p-8 rounded-[32px] border border-border hover:border-primary-500 hover:shadow-2xl hover:shadow-primary-500/10 hover:-translate-y-2 transition-all duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
            ...
            <div className="lg:col-span-5 space-y-8">
              ...
              <div className="space-y-4 pt-2">
                {[
                  ...
                  <div key={i} className="flex items-center space-x-4 text-base font-black text-foreground">
                    <CheckCircle2 className="w-5 h-5 text-success shrink-0" />
                    <span>{item}</span>
                  </div>
                ))}
            ...
            <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6">
              {siteConfig.trustIndicators.map((ind, i) => (
                <div key={i} className="group p-8 bg-surface rounded-[32px] border border-border shadow-xl shadow-foreground/5 dark:border-slate-800 dark:shadow-slate-950/20 flex items-start space-x-5 transition-all duration-500 hover:border-primary-500 hover:shadow-2xl hover:shadow-primary-500/10 hover:-translate-y-1">
        ...
        <div className="max-w-7xl mx-auto space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {siteConfig.testimonials.map((t) => (
              <div key={t.id} className="p-8 sm:p-10 rounded-[40px] bg-surface border border-border flex flex-col justify-between relative overflow-hidden shadow-xl shadow-foreground/5 transition-all duration-500 hover:shadow-2xl hover:border-primary-400">
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div className={`p-2 rounded-xl transition-all duration-300 ${isOpen ? 'bg-primary-600 text-white' : 'bg-muted text-primary-600'}`}>
